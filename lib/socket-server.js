...
function initializeSocketServer(server) {
  ...
  io.use(async (socket, next) => {
    try {
      // Support token from auth or query and strip possible 'Bearer ' prefix
...
function handleChatEvents(socket) {
  ...
  socket.on('send_message', async (data) => {
    try {
      const { conversationId, messageType, content, fileUrl, fileName, fileSize, replyTo, senderName, clientTempId } = data
      ...
      const updatedConversation = { ...conversation, lastMessageAt, lastMessage: message }
      for (const participantId of conversation.participants || []) {
        io.to(`user_${participantId}`).emit('conversation_updated', updatedConversation)
      }
