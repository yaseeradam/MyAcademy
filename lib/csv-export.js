...
export const exportTeacherAttendanceToCSV = (attendance, teachers, schoolName) => {
  const exportData = attendance.map(record => {
    const teacher = teachers.find(t => t.id === record.teacherId)
    const teacherName = teacher ? `${teacher.firstName || ''} ${teacher.lastName || ''}`.trim() : 'Unknown'
    const recordDate = record.date || record.createdAt
    return {
      'Teacher Name': teacherName,
      'Teacher ID': record.teacherId || '',
      'Status': record.status || '',
      'Date': recordDate ? new Date(recordDate).toLocaleDateString() : '',
      'Time': record.createdAt ? new Date(record.createdAt).toLocaleTimeString() : '',
      'Marked By': record.markedBy || ''
    }
  })

  exportToCSV(exportData, 'teacher_attendance', schoolName)
}

export const exportStudentAttendanceToCSV = (attendance, students, classes, schoolName) => {
  const exportData = attendance.map(record => {
  ...
  exportToCSV(exportData, 'student_attendance', schoolName)
}
