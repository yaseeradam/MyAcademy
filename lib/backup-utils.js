...
export async function buildBackupPayload(db, schoolId) {
  console.log('[Backup] Starting backup for school:', schoolId)
  const collections = await listBackupCollections(db, schoolId)
  console.log('[Backup] Found collections to backup:', collections.length)
  const data = {}

  for (const name of collections) {
    console.log('[Backup] Processing collection:', name)
    const startTime = Date.now()
    try {
      const sampleDoc = await db.collection(name).findOne()
      const filter = getCollectionFilter(name, sampleDoc, schoolId)
      if (!filter) {
        console.log('[Backup] Skipped collection (no filter):', name)
        continue
      }
      const docs = await db.collection(name).find(filter).maxTimeMS(30000).toArray()
      if (docs.length) {
        data[name] = docs
        console.log(`[Backup] Backed up ${docs.length} docs from ${name} in ${Date.now() - startTime}ms`)
      }
    } catch (error) {
      console.error(`[Backup] Error backing up collection ${name}:`, error.message)
      // Continue with other collections instead of failing entirely
    }
  }

  console.log('[Backup] Backup complete. Total collections backed up:', Object.keys(data).length)
  return {
    meta: {
      schoolId,
