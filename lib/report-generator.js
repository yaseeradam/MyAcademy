...
export const generateClassReportCards = (students, subjects, allScores, schoolName, className, gradingScale = []) => {
  const doc = new jsPDF()
  ...
  students.forEach((student, index) => {
    ...
    doc.text('OFFICIAL REPORT CARD', 105, 35, { align: 'center' })
    ...
    doc.setFontSize(11)
    doc.text(`Name: ${student.firstName} ${student.lastName}`, 20, 55)
    doc.text(`Admission No: ${student.admissionNumber || 'N/A'}`, 120, 55)
    doc.text(`Class: ${className}`, 20, 65)
    doc.text(`Session: ${new Date().getFullYear()}`, 120, 65)
    doc.text(`Gender: ${student.gender || 'N/A'}`, 20, 75) // Gender
