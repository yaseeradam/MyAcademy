...
// GET - Preview promotion or get history
export async function GET(request) {
    try {
        ...
        // Preview promotion
        if (!academicYear) {
            return NextResponse.json({ error: 'Academic year is required for preview' }, { status: 400 })
        }

        // Get all active students for the school
        const students = await db.collection('students')
            .find({ schoolId: user.schoolId, status: { $ne: 'graduated' } })
            .toArray()

        // Get all classes with their promotion settings
        ...
        // Calculate suggested action for each student
        const studentsWithSuggestion = students.map(student => {
            const currentClass = classMap.get(student.classId)
            let suggestedAction = 'promote'

            if (!currentClass) {
                suggestedAction = 'promote' // Default if class not found
            } else if (currentClass.isFinalClass) {
                suggestedAction = 'graduate'
            } else if (!currentClass.nextClassId) {
                suggestedAction = 'repeat' // No next class configured
            }
            ...
            return {
                ...
                nextClassId: currentClass?.nextClassId,
                nextClassName: currentClass?.nextClassId ? classMap.get(currentClass.nextClassId)?.name : null,
                isFinalClass: currentClass?.isFinalClass || false,
                suggestedAction
            }
        })

        // Group by class for easier review
        const byClass = {}
        studentsWithSuggestion.forEach(student => {
            if (!byClass[student.classId]) {
                byClass[student.classId] = {
                    className: student.className,
                    nextClassName: student.nextClassName,
                    isFinalClass: student.isFinalClass,
                    students: []
                }
            }
            byClass[student.classId].students.push(student)
        })

        return NextResponse.json({
            academicYear,
            totalStudents: students.length,
            students: studentsWithSuggestion,
            byClass,
